...
mod cryptography_tests {
    ...
    fn kdf() {
        ...
        let kdf = kdf::key_derivation(&ikm[..], Some(&salt[..]), &info[..], 3 * CRYPTO_SYMMETRIC_KEY_LENGTH_BITS);
    }
    ...
    fn rfc_spake2() {
        for rfc in RFC_T {
            ...
            let p_b = rfc.Y;
            assert_eq!(
                &spake.compute_public_prover(&rfc.w0).unwrap().to_encoded_point(false).to_bytes().to_vec()[..],
                p_a
            );
            assert_ne!(
                &spake
                    .compute_public_verifier(&rfc.w0)
                    .unwrap()
            ...
            assert_eq!(confirmation.k_e, rfc.Ke);
            assert_eq!(confirmation.c_a, rfc.cA);
        ...
        for test in get_test_vectors() {
            ...
            let p_a = spake
                .compute_public_prover(&prover_values.w0)
                .unwrap()
                .to_encoded_point(false)
                .to_bytes()
                .to_vec();
            let p_b = spake
                .compute_public_verifier(&verifier_values.w0)
                .unwrap()
                .to_encoded_point(false)
                .to_bytes()
                .to_vec();
            ...
            assert_eq!(hex::decode(test.HMAC_K_confirmP_shareV).unwrap(), confirmation.c_a);
    ...
    fn matter_spake2() {
        ...
        assert_eq!(
            prover.w0.to_vec(),
            hex::decode("00177867f1e564cc4d9f347edfc28263ee5a50f1e21177cfb9a7dc2504437ccb").unwrap()
        );
        assert_eq!(
            prover.w1.to_vec(),
            hex::decode("0e60dc5cc1bb4b66b4547601a95bc6a1758a79f79f24f68823773fb1bb9791ca").unwrap()
        );
        assert_eq!(
            verifier.L.to_vec(),
            hex::decode(
        ...
        assert_eq!(
            z.to_encoded_point(false).as_bytes(),
            hex::decode(
        ...
        assert_eq!(
            confirmation.cB.to_vec(),
            hex::decode("d6a13c26b6c5b7c514033a0370b1830dff5116fd53de43eb2374737e9b64e4bb").unwrap()
        );
        assert_eq!(
            confirmation.c_a.to_vec(),
            hex::decode("05030c97e72b884cd0ebf0f2e6f91bfd377a8f37601bb7f633d468e289437cf7").unwrap()
        );

        // // MatterJS Test Case 2 (PasePairingTest)
        let mut spake = Spake2P::new();
        let param_set: PBKDFParameterSet = PBKDFParameterSet {
            iterations: 1000,
            salt: hex::decode("2bb41e9d75f30c2e6b2f059410c56965717cc2bf14ed6c73a169435326a89652")
                .unwrap()
                .try_into()
                .unwrap(),
        };
        ...
        assert_eq!(
            hex::encode(verifier.w0),
            "501f85a83d1da77983ff6f0c1f742d6d98f6d0ab0ba740a38032200099c8981f"
        );
        assert_eq!(
            hex::encode(verifier.L),
...
fn spake2p_confirmation() {
    ...
    assert_eq!(
        ...
        "05030c97e72b884cd0ebf0f2e6f91bfd377a8f37601bb7f633d468e289437cf7"
    );
    assert_eq!(
        hex::encode(confirmation.cB),
        "d6a13c26b6c5b7c514033a0370b1830dff5116fd53de43eb2374737e9b64e4bb"
    );
