...
fn signed_integers() {
    ...
    assert_eq!(tlv_as_hex(Signed64(40000000000)), "03 00 90 2f 50 09 00 00 00");
...
fn strings() {
    ...
    assert_eq!(tlv_as_hex(OctetString8(vec![00, 01, 02, 03, 04])), "10 05 00 01 02 03 04");
...
fn single_precision_floating_points() {
    ...
    assert_eq!(tlv_as_hex(FloatingPoint4(f32::NEG_INFINITY)), "0a 00 00 80 ff");
...
fn double_precision_floating_points() {
    ...
    assert_eq!(tlv_as_hex(FloatingPoint8(f64::NEG_INFINITY)), "0b 00 00 00 00 00 00 f0 ff");
...
pub fn vendor() {
    ...
    assert_eq!(
        as_hex_string(&parse_tlv(&[0x64, 0xa0, 0x86, 0x01, 0x00, 0x2a]).to_bytes()),
        "64 a0 86 01 00 2a"
    );
    ...
    assert_eq!(
        as_hex_string(&parse_tlv(&[0xc4, 0xf1, 0xff, 0xed, 0xde, 0x01, 0x00, 0x2a]).to_bytes()),
        "c4 f1 ff ed de 01 00 2a"
    );
    ...
    assert_eq!(
        as_hex_string(&parse_tlv(&[0xe4, 0xf1, 0xff, 0xed, 0xde, 0xed, 0xfe, 0x55, 0xaa, 0x2a]).to_bytes()),
        "e4 f1 ff ed de ed fe 55 aa 2a"
    );
    ...
    assert_eq!(
        as_hex_string(&parse_tlv(&[0xd5, 0xf1, 0xff, 0xed, 0xde, 0x01, 0x00, 0xc4, 0xf1, 0xff, 0xed, 0xde, 0x55, 0xaa, 0x2a, 0x18]).to_bytes()),
        "d5 f1 ff ed de 01 00 c4 f1 ff ed de 55 aa 2a 18"
    );
}
