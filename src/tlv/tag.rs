...
impl From<Tag> for Vec<u8> {
    fn from(tag: Tag) -> Vec<u8> {
        ...
        if let Some(profile) = tag.profile {
            data.write_u16::<LE>(profile).expect("Unable to write vendor id...");
        }
        ...
        if let Some(tag) = tag.tag_number {
            match tag {
                ...
                TagNumber::Medium(number) => {
                    data.write_u16::<LE>(number).expect("Unable to write medium tag number...");
                }
                TagNumber::Long(number) => {
                    data.write_u32::<LE>(number).expect("Unable to write long tag number...");
                }
            }
        }
